<?php
// ============================================================
//  ThreatBadger — includes/sources.php
...
function query_misp(string $indicator, string $type, array $cfg): array {
    $type_map = [
        ...
        'IPv6 Address' => ['ip-src','ip-dst','ip-src|port','ip-dst|port'],
...
function query_virustotal(string $indicator, string $type, string $api_key): array {
    $endpoints = [
        ...
        'IPv6 Address' => "https://www.virustotal.com/api/v3/ip_addresses/$indicator",
    ...
    $gui_type = $type === 'IPv4 Address' ? 'ip-address' : ($type === 'IPv6 Address' ? 'ip-address' : ($type === 'Domain' ? 'domain' : 'file'));
    return [
        'source'     => 'VirusTotal',
        'found'      => true,
        'verdict'    => $verdict,
...
function query_greynoise(string $indicator, string $type, string $api_key): array {
    if ($type !== 'IPv4 Address' && $type !== 'IPv6 Address') {
        return ['source' => 'GreyNoise', 'skipped' => true, 'reason' => 'GreyNoise only supports IP Address lookups'];
    }
 ...
 */
function query_email_blacklists(string $email, ?string $hibp_key = null, ?string $emailrep_key = null): array {
    ...
    if ($surbl_hit) {
        $matches[]  = [
            ...
            'detail'   => "$domain listed in SURBL multi blocklist", 
            'severity' => 'high',
            'tags'     => ['SURBL']
        ];
        $verdicts[] = 'MALICIOUS';
    ...
    $dbl_lookup = $domain . '.dbl.spamhaus.net';
    ...
    if ($sfs_res['ok'] && isset($sfs_res['data']['email'])) {
        ...
        if (!empty($sfs['appears'])) {
            ...
            $matches[]  = [
                'list'     => 'StopForumSpam', 
                'type'     => 'Spam/Abuse registry', 
                'detail'   => "Reported $freq time(s), last seen $lastseen", 
                'severity' => 'medium',
                'tags'     => ['StopForumSpam']
            ];
    ...
    $erep_data = null;
    if ($emailrep_key) {
      $erep_res = http_get("https://emailrep.io/" . urlencode($email), ['User-Agent' => 'ThreatBadger/1.0', 'Key' => $emailrep_key]);
      if ($erep_res['ok'] && is_array($erep_res['data'])) {
        ...
        if ($erep_data['blacklisted'] || $erep_data['malicious_activity']) {
            $matches[]  = [
                ...
                'detail'   => "Reputation: {$erep_data['reputation']} — blacklisted/malicious activity flagged", 
                'severity' => 'high',
                'tags'     => ['EmailRep.io']
            ];
            $verdicts[] = 'MALICIOUS';
            ...
            $matches[]  = [
                'list'     => 'EmailRep.io', 
                'type'     => 'Email reputation', 
                'detail'   => "Reputation: {$erep_data['reputation']} — flagged as suspicious/spam", 
                'severity' => 'medium',
                'tags'     => ['EmailRep.io']
            ];
            $verdicts[] = 'SUSPICIOUS';
            ...
            $matches[]  = [
                ...
                'detail'   => "Disposable/temporary email provider detected", 
                'severity' => 'low',
                'tags'     => ['EmailRep.io']
            ];
            $verdicts[] = 'SUSPICIOUS';
        ...
        if ($erep_data['credentials_leaked'] || $erep_data['data_breach']) {
            $matches[] = [
                ...
                'detail'   => 'Credentials or data associated with this email have been leaked', 
                'severity' => 'medium',
                'tags'     => ['EmailRep.io']
            ];
    ...
    if ($hibp_key) {
        $hibp_res = http_get(
            ...
            ['hibp-api-key' => $hibp_key, 'User-Agent' => 'ThreatBadger/1.0']
        );
...
// ─── Custom Lists ─────────────────────────────────────────────
/**
 * Query all configured CUSTOM_LISTS for an indicator.
 */
function query_custom_lists(string $indicator, string $type): array {
    if (!defined('CUSTOM_LISTS') || empty(CUSTOM_LISTS)) {
        return ['source' => 'Custom Lists', 'found' => false, 'skipped' => true,
                'reason' => 'No custom lists configured', 'matches' => [], 'list_results' => []];
    }

    // Map indicator type to which list types are relevant
    $relevant_list_types = [];
    switch ($type) {
        case 'IPv4 Address':
        case 'IPv6 Address':
            $relevant_list_types = ['IP'];
            break;
        case 'Domain':
            $relevant_list_types = ['DOMAIN'];
            break;
        case 'Email Address':
            $relevant_list_types = ['EMAIL'];
            break;
        case 'MD5 Hash':
        case 'SHA-1 Hash':
        case 'SHA-256 Hash':
            $relevant_list_types = ['HASH'];
            break;
    }
    ...
    foreach (CUSTOM_LISTS as $list) {
        $list_name    = $list['name']    ?? 'Unnamed List';
        $list_type    = strtoupper($list['type'] ?? '');
        $list_url     = $list['url']     ?? '';
        $list_scoring = strtolower($list['scoring'] ?? 'threats'); // 'threats' | 'information'

        // Skip lists that don't apply to this indicator type
        if (!in_array($list_type, $relevant_list_types, true)) {
            continue;
        }

        // Fetch the list
        $lines = custom_list_fetch($list_url);
        if ($lines === null) {
            $list_results[] = [
                'list_name' => $list_name,
                'list_type' => $list_type,
                'found'     => false,
                'error'     => 'Failed to fetch list from URL',
                'matches'   => [],
            ];
            continue;
        }
        ...
        $hits = [];
        if ($list_type === 'IP') {
            $hits = custom_list_search_ip($indicator, $lines);
        } elseif ($list_type === 'DOMAIN') {
            $hits = custom_list_search_domain($indicator, $lines);
        } elseif ($list_type === 'EMAIL' || $list_type === 'HASH') {
            // Exact match (case-insensitive)
            $ind_lower = strtolower($indicator);
            foreach ($lines as $line) {
                $line_clean = strtolower(trim($line));
                if ($line_clean !== '' && $line_clean === $ind_lower) {
                    $hits[] = $line_clean;
                }
            }
        }
        ...
        if ($found) {
            foreach ($hits as $hit) {
                $all_matches[] = [
                    ...
                    'type'       => $list_type,
                    'list_scoring' => $list_scoring,
                    'detail'     => "Matched in list: {$list_name}",
                    'tags'       => [],
                ];
            }
        }
    ...
    $total_hits = count($all_matches);
    // Only threat-scoring matches count toward the verdict/score
    $threat_hits = count(array_filter($all_matches, fn($m) => ($m['list_scoring'] ?? 'threats') === 'threats'));
    $found = $total_hits > 0;

    // Verdict: based on threat hits only
    $verdict = 'CLEAN';
    if ($threat_hits >= 2) $verdict = 'MALICIOUS';
    elseif ($threat_hits === 1) $verdict = 'SUSPICIOUS';

    return [
        'source'       => 'Custom Lists',
        'found'        => $found,
        'verdict'      => $verdict,
        'match_count'  => $threat_hits,
        'matches'      => $all_matches,
        'list_results' => $list_results,
        'summary'      => $found
            ? "{$total_hits} match(es) across custom lists ({$threat_hits} threat) — {$verdict}"
            : 'No matches in custom lists',
    ];
}

/**
 * Fetch a plain-text list from a URL, return array of non-empty, non-comment lines.
 * Returns null on HTTP/curl failure.
 */
function custom_list_fetch(string $url): ?array {
    $ch = curl_init();
    curl_setopt_array($ch, [
        ...
        CURLOPT_TIMEOUT        => 10,
        CURLOPT_USERAGENT      => 'ThreatBadger/1.3',
    ...
    if ($body === false || !empty($err) || $status >= 400) {
        return null;
    }

    $lines = explode("\n", $body);
    // Strip comment lines and blank lines
    return array_filter(array_map('trim', $lines), function(string $l): bool {
        return $l !== '' && $l[0] !== '#' && $l[0] !== ';';
    });
}

/**
 * Search a list of strings for an IP address.
 * Handles both plain IPs (exact match) and CIDR notation.
 */
function custom_list_search_ip(string $ip, array $lines): array {
    $hits     = [];
    $ip_long  = ip2long($ip);
    $is_ipv4  = $ip_long !== false;
    ...
    foreach ($lines as $line) {
        ...
        if (strpos($entry, '/') !== false) {
            // CIDR range — only works for IPv4 currently
            if ($is_ipv4 && custom_ip_in_cidr($ip_long, $entry)) {
                $hits[] = $entry;
            }
        } else {
            // Plain IP — exact match
            if (strtolower($entry) === strtolower($ip)) {
                $hits[] = $entry;
            }
        }
    }
...
/**
 * Returns true if an IPv4 address (as long) falls inside a CIDR range string.
 */
function custom_ip_in_cidr(int $ip_long, string $cidr): bool {
    ...
    if ($net_long === false) return false;
    $mask = $prefix === 0 ? 0 : (~0 << (32 - $prefix));
    return ($ip_long & $mask) === ($net_long & $mask);
}

/**
 * Search a list for a domain using substring matching (both directions).
 * e.g. query "evil.com" matches "evil.com", "sub.evil.com", "evil.com/path"
 *      and query "sub.evil.com" matches list entry "evil.com"
 */
function custom_list_search_domain(string $domain, array $lines): array {
    $hits   = [];
    $domain = strtolower(trim($domain));
...
// ─── HackerTarget ─────────────────────────────────────────────
/**
 * Query HackerTarget API for enrichment data.
 * IPs:     reversedns + reverseip
 * Domains: dnslookup + hostsearch
 * Emails:  extract domain, then dnslookup + hostsearch
 *
 * This source is INFORMATIONAL only — results never contribute to threat scoring.
 */
function query_hackertarget(string $indicator, string $type, string $api_key = ''): array {
    ...
    // Determine the actual query target and which endpoints to call
    $is_ip     = in_array($type, ['IPv4 Address', 'IPv6 Address'], true);
    $is_domain = $type === 'Domain';
    $is_email  = $type === 'Email Address';

    $query_target = $indicator;
    if ($is_email) {
        // Extract domain from email
        $parts        = explode('@', $indicator, 2);
        $query_target = $parts[1] ?? $indicator;
    }

    // ── IP lookups ────────────────────────────────────────────
    if ($is_ip) {
        // Reverse DNS
        $rdns = ht_fetch("{$base}/reversedns/?q=" . urlencode($query_target), $headers);
        if ($rdns['ok']) {
            $lines = ht_parse_lines($rdns['body']);
            if (!empty($lines) && !ht_is_error($lines[0])) {
                $found = true;
                $sections['reverse_dns'] = $lines;
            } elseif (!empty($lines)) {
                $errors[] = 'Reverse DNS: ' . $lines[0];
            }
        } else {
            $errors[] = 'Reverse DNS fetch failed (HTTP ' . $rdns['status'] . ')';
        }

        // Reverse IP (other domains on same IP)
        $rip = ht_fetch("{$base}/reverseiplookup/?q=" . urlencode($query_target), $headers);
        if ($rip['ok']) {
            $lines = ht_parse_lines($rip['body']);
            if (!empty($lines) && !ht_is_error($lines[0])) {
                $found = true;
                $sections['reverse_ip'] = $lines;
            } elseif (!empty($lines)) {
                $errors[] = 'Reverse IP: ' . $lines[0];
            }
        } else {
            $errors[] = 'Reverse IP fetch failed (HTTP ' . $rip['status'] . ')';
        }
    }

    // ── Domain / Email lookups ────────────────────────────────
    if ($is_domain || $is_email) {
        // DNS Lookup
        $dns = ht_fetch("{$base}/dnslookup/?q=" . urlencode($query_target), $headers);
        if ($dns['ok']) {
            $lines = ht_parse_lines($dns['body']);
            if (!empty($lines) && !ht_is_error($lines[0])) {
                $found = true;
                $sections['dns_records'] = $lines;
            } elseif (!empty($lines)) {
                $errors[] = 'DNS Lookup: ' . $lines[0];
            }
        } else {
            $errors[] = 'DNS Lookup fetch failed (HTTP ' . $dns['status'] . ')';
        }

        // Host Search (domains hosted on same IPs)
        $hs = ht_fetch("{$base}/hostsearch/?q=" . urlencode($query_target), $headers);
        if ($hs['ok']) {
            $lines = ht_parse_lines($hs['body']);
            if (!empty($lines) && !ht_is_error($lines[0])) {
                $found = true;
                $sections['host_search'] = $lines;
            } elseif (!empty($lines)) {
                $errors[] = 'Host Search: ' . $lines[0];
            }
        } else {
            $errors[] = 'Host Search fetch failed (HTTP ' . $hs['status'] . ')';
        }
    }

    $error_msg  = !empty($errors) ? implode('; ', $errors) : null;
    $query_note = $is_email ? "domain extracted from email: {$query_target}" : null;

    $summary = $found
        ? 'HackerTarget data found (' . implode(', ', array_keys($sections)) . ')'
        : ($error_msg ?: 'No data returned');

    return [
        'source'      => 'HackerTarget',
        'found'       => $found,
        'verdict'     => 'INFO',          // never scored
        'informational' => true,
        'query_target'  => $query_target,
        'query_note'    => $query_note,
        'sections'      => $sections,
        'error_message' => $error_msg,
        'matches'       => [],            // no IOC matches
        'summary'       => $summary,
        'enrichment'    => ['tags' => []],
    ];
}

/** HTTP GET for HackerTarget — returns ['ok', 'status', 'body'] */
function ht_fetch(string $url, array $headers = []): array {
    $ch = curl_init();
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_FOLLOWLOCATION => true,
        CURLOPT_TIMEOUT        => 15,
        CURLOPT_USERAGENT      => 'ThreatBadger/' . APP_VERSION,
        CURLOPT_SSL_VERIFYPEER => false,
    ...
    curl_close($ch);
    return [
        'ok'     => $body !== false && empty($err) && $status < 400,
        'status' => $status,
        'body'   => (string)($body ?: ''),
    ];
}

/** Split body into non-empty, non-comment lines */
function ht_parse_lines(string $body): array {
...
/** Detect HackerTarget API error responses */
function ht_is_error(string $line): bool {
    $lower = strtolower($line);
    return str_contains($lower, 'error') ||
           str_contains($lower, 'api count exceeded') ||
           str_contains($lower, 'invalid') ||
           str_contains($lower, 'denied') ||
           str_contains($lower, 'limit') ||
           str_contains($lower, 'upgrade');
}

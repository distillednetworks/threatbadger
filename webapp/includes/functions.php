<?php
// ============================================================
//  ThreatBadger — includes/functions.php
...
/**
 * Map Okta groups array to a ThreatBadger role using config group_role_map.
 */
...
function http_get(string $url, array $headers = [], bool $verify_ssl = true, int $timeout = 20): array {
    return http_request('GET', $url, null, $headers, $verify_ssl, $timeout);
}

function http_post(string $url, $body, array $headers = [], bool $verify_ssl = true, int $timeout = 20): array {
...
function detect_type(string $indicator): ?string {
    ...
    if (preg_match('/^(?:(?:[0-9a-fA-F]{1,4}:){7}[0-9a-fA-F]{1,4}|(?:[0-9a-fA-F]{1,4}:){1,7}:|:(?::[0-9a-fA-F]{1,4}){1,7}|(?:[0-9a-fA-F]{1,4}:){1,6}:[0-9a-fA-F]{1,4}|(?:[0-9a-fA-F]{1,4}:){1,5}(?::[0-9a-fA-F]{1,4}){1,2}|(?:[0-9a-fA-F]{1,4}:){1,4}(?::[0-9a-fA-F]{1,4}){1,3}|(?:[0-9a-fA-F]{1,4}:){1,3}(?::[0-9a-fA-F]{1,4}){1,4}|(?:[0-9a-fA-F]{1,4}:){1,2}(?::[0-9a-fA-F]{1,4}){1,5}|[0-9a-fA-F]{1,4}:(?:(?::[0-9a-fA-F]{1,4}){1,6})|:(?:(?::[0-9a-fA-F]{1,4}){1,6}))$/', $q)) return 'IPv6 Address';
...
function history_save(array $data): bool {
    ...
    if (!is_dir($dir)) {
        if (!mkdir($dir, 0750, true)) {
            error_log('[ThreatBadger] history_save: cannot create directory ' . $dir);
            return false;
        }
    }
    if (!is_writable($dir) && !(file_exists(HISTORY_FILE) && is_writable(HISTORY_FILE))) {
        error_log('[ThreatBadger] history_save: ' . HISTORY_FILE . ' is not writable by web server');
        return false;
    }
    ...
    if ($json === false) {
        error_log('[ThreatBadger] history_save: json_encode failed ‚ ' . json_last_error_msg());
        return false;
    }
    $written = file_put_contents(HISTORY_FILE, $json, LOCK_EX);
    if ($written === false) {
        error_log('[ThreatBadger] history_save: file_put_contents failed for ' . HISTORY_FILE);
        return false;
    }
    return true;
}
...
function settings_load(): array {
    $defaults = [
        ...
        'hibp'          => ['api_key' => HIBP_API_KEY, 'enabled' => false],
        'emailrep'      => ['api_key' => EMAILREP_API_KEY, 'enabled' => false],
        'whois'         => ['enabled' => true],
        'custom_lists'  => ['enabled' => true],
        'hackertarget'  => ['api_key' => defined('HACKERTARGET_API_KEY') ? HACKERTARGET_API_KEY : '', 'enabled' => true],
    ];
    ...
    $defaults['hibp']['enabled']          = !empty($defaults['hibp']['api_key']);
    $defaults['emailrep']['enabled']      = !empty($defaults['emailrep']['api_key']);
    // HackerTarget is always enabled (free tier works without key; key increases rate limits)
    $defaults['hackertarget']['enabled']  = true;
    return $defaults;
}

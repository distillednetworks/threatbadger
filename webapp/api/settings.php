...
if ($method === 'GET' && !$action) {
    ...
    if (!empty($s['hibp']['api_key']))          $s['hibp']['api_key']          = mask_key($s['hibp']['api_key']);
    if (!empty($s['hackertarget']['api_key']))  $s['hackertarget']['api_key']  = mask_key($s['hackertarget']['api_key']);
//    json_response($s);
...
if ($method === 'POST' && $action === 'test') {
    ...
    switch ($source) {
        ...
        case 'hibp':
            if (empty($s['hibp']['api_key'])) json_response(['success' => false, 'message' => 'API key required']);
            $res = http_get('https://api.greynoise.io/user@domain.com', ['key' => $s['hibp']['api_key']]); // Still to do this
            json_response(['success' => $res['ok'], 'message' => $res['ok'] ? 'HiBP API key valid' : 'Invalid API key: HTTP ' . $res['status']]);
            break;

        case 'hackertarget':
            $ht_key = $s['hackertarget']['api_key'] ?? '';
            $url    = 'https://api.hackertarget.com/dnslookup/?q=hackertarget.com' . (!empty($ht_key) ? '&apikey=' . urlencode($ht_key) : '');
            $ch     = curl_init();
            curl_setopt_array($ch, [CURLOPT_URL => $url, CURLOPT_RETURNTRANSFER => true, CURLOPT_TIMEOUT => 10, CURLOPT_SSL_VERIFYPEER => false, CURLOPT_USERAGENT => 'ThreatBadger/' . APP_VERSION]);
            $body   = curl_exec($ch);
            $status = (int)curl_getinfo($ch, CURLINFO_HTTP_CODE);
            curl_close($ch);
            $is_error = $body === false || $status >= 400 || (stripos((string)$body, 'error') !== false && stripos((string)$body, 'api count') !== false);
            json_response(['success' => !$is_error, 'message' => !$is_error ? 'HackerTarget reachable' . (!empty($ht_key) ? ' (registered key)' : ' (free tier)') : 'HackerTarget unreachable: HTTP ' . $status]);
            break;

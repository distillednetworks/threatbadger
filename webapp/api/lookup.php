...
// ─── Define the varialbles for non-IPv4 Address ──────────────────────────────────────
$geo_result   = null;
$whois_result = null;
...
if ($is_email) {
    try {
        ...
        $emailrep_key = $settings['emailrep']['api_key'] ?? null;
        $results[] = query_email_blacklists($indicator, $hibp_key ?: null, $emailrep_key ?: null);
    }
...
// GeoIP — IPv6 only
if ($type === 'IPv6 Address' && $settings['geoip']['enabled'] && $source_active('geoip')) {
    try { $geo_result = query_geoip($indicator, $settings['geoip']); }
    catch (Exception $e) { $geo_result = ['source' => 'GeoIP', 'found' => false, 'error' => $e->getMessage()]; }
}
...
// HackerTarget — informational only (IPv4, IPv6, Domain, Email)
$hackertarget_result = null;
$ht_applicable_types = ['IPv4 Address', 'IPv6 Address', 'Domain', 'Email Address'];
if ($settings['hackertarget']['enabled'] && in_array($type, $ht_applicable_types, true) && $source_active('hackertarget')) {
    try {
        $ht_api_key = $settings['hackertarget']['api_key'] ?? '';
        $hackertarget_result = query_hackertarget($indicator, $type, $ht_api_key);
    }
    catch (Exception $e) { $hackertarget_result = ['source' => 'HackerTarget', 'found' => false, 'informational' => true, 'error_message' => $e->getMessage(), 'sections' => [], 'matches' => [], 'summary' => $e->getMessage()]; }
}

// Custom Lists — always run if configured (no API key needed, user can toggle via badge)
$custom_list_result = null;
if (defined('CUSTOM_LISTS') && !empty(CUSTOM_LISTS) && $source_active('custom_lists')) {
    try { $custom_list_result = query_custom_lists($indicator, $type); }
    catch (Exception $e) { $custom_list_result = source_error('Custom Lists', 0, $e->getMessage()); }
    if ($custom_list_result) $results[] = $custom_list_result;
}

if (empty($results) && empty($geo_result) && empty($whois_result) && empty($hackertarget_result)) {
    json_error('No intelligence sources configured. Add sources to config.php file.');
}

// ─── Aggregate ────────────────────────────────────────────────
$score      = calculate_score($results);

// Custom list score boost: 1 match = at least SUSPICIOUS (25), 2+ = at least MALICIOUS (50)
//if (!empty($custom_list_result) && $custom_list_result['found'] ?? false) {
//    $cl_hits = $custom_list_result['match_count'] ?? 0;
//    if ($cl_hits >= 2 && $score < 50) $score = max($score, 50);
//    elseif ($cl_hits === 1 && $score < 25) $score = max($score, 25);
//}
...
$payload = [
    ...
    'tags'       => $tags,
    'summary'    => [
        'sources_queried'    => count($results) + (!empty($geo_result) ? 1 : 0) + (!empty($whois_result) ? 1 : 0) + (!empty($hackertarget_result) ? 1 : 0),
    ...
    'hackertarget' => $hackertarget_result ?? null,
    'file_intel'  => ($is_hash && $vt_result) ? $vt_result['enrichment'] : null,
    'vt_stats'    => $vt_result['stats'] ?? null,
    'custom_list_results' => $custom_list_result['list_results'] ?? [],
];

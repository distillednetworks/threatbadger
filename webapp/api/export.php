...
if ($format === 'csv') {
    ...
    header('Content-Disposition: attachment; filename="ThreatBadger-history-' . date('Ymd-His') . '.csv"');
...
if ($format === 'json' && $id) {
    ...
    header('Content-Disposition: attachment; filename="ThreatBadger-' . $safe_name . '.json"');
...
<title>ThreatBadger Report — {$safe_q}</title>
<style>
...
<div class="header">
  <div><h1>🔭 ThreatBadger</h1><div style="font-size:10px;color:#8b949e;margin-top:3px">Threat Intelligence Report</div></div>
...
HTML;
    ...
    echo '<div class="footer">ThreatBadger · For authorized use only · Handle as TLP:AMBER</div></body></html>';
}
...
function generate_tcpdf(array $item): void {
    ...
    $pdf->SetCreator('ThreatBadger');
    $pdf->SetAuthor($item['user']);
    $pdf->SetTitle('ThreatBadger Report — ' . $item['query']);
    $pdf->setPrintHeader(false);
    $pdf->setPrintFooter(false);
    $pdf->AddPage();
    $pdf->SetFont('helvetica','B',14);
    $pdf->SetTextColor(88,166,255);
    $pdf->Cell(0,10,'ThreatBadger — Threat Intelligence Report',0,1,'L');
    $pdf->SetFont('helvetica','',10);
    $pdf->SetTextColor(100,100,100);
    $pdf->Cell(0,6,'Indicator: ' . $item['query'] . '   |   Risk: ' . $r['risk_level'] . '   |   Score: ' . $r['score'],0,1,'L');
    $pdf->Output('ThreatBadger-' . preg_replace('/[^a-z0-9]/i','_',$item['query']) . '.pdf','D');
}

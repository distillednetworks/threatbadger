<?php
// ============================================================
//  ThreatBadger — saml/acs.php  (Assertion Consumer Service)
//
...
//    3. Maps Okta groups → ThreatBadger role
...
try {
    ...
    if (!empty($errors)) {
        ...
        error_log('[ThreatBadger SAML] ACS errors: ' . implode(', ', $errors) . ' — ' . $detail);
    ...
    // ── Map Okta groups to ThreatBadger role ────────────────────
    ...
    error_log("[ThreatBadger SAML] Login: $username ($role) from " . ($_SERVER['REMOTE_ADDR'] ?? 'unknown'));
    ...
    error_log('[ThreatBadger SAML] ACS exception: ' . $e->getMessage());

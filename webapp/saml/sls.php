<?php
// ============================================================
//  ThreatBadger — saml/sls.php  (Single Logout Service)
//
//  Handles both SP-initiated and IdP-initiated Single Logout.
//
//  SP-initiated: User clicks logout in ThreatBadger →
...
try {
    ...
    $auth->processSLO(
        ...
        function() use ($username) {
            ...
            error_log("[ThreatBadger SAML] SLO complete for: $username");
        }
    );

    $errors = $auth->getErrors();
    if (!empty($errors)) {
        error_log('[ThreatBadger SAML] SLO errors: ' . implode(', ', $errors));
    }
    ...
    error_log('[ThreatBadger SAML] SLS exception: ' . $e->getMessage());
